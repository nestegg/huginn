...
describe Huginn::ScenarioImport do
  let(:user) { users(:bob) }
  ...
  let(:valid_parsed_weather_agent_data) do
    {
      :type => "Huginn::Agents::WeatherAgent",
  ...
  let(:valid_parsed_trigger_agent_data) do
    {
      :type => "Huginn::Agents::TriggerAgent",
  ...
  let(:valid_parsed_basecamp_agent_data) do
    {
      :type => "Huginn::Agents::BasecampAgent",
      :name => "Basecamp test",
      :schedule => "every_2m",
      :keep_events_for => 0,
      :propagate_immediately => true,
  ...
  describe "initialization" do
    it "is initialized with an attributes hash" do
      expect(Huginn::ScenarioImport.new(:url => "http://google.com").url).to eq("http://google.com")
    end
  end

  describe "validations" do
    subject do
      _import = Huginn::ScenarioImport.new
  ...
  describe "#dangerous?" do
    it "returns false on most Agents" do
      expect(Huginn::ScenarioImport.new(:data => valid_data)).not_to be_dangerous
    end

    it "returns true if a ShellCommandAgent is present" do
      valid_parsed_data[:agents][0][:type] = "Huginn::Agents::ShellCommandAgent"
      expect(Huginn::ScenarioImport.new(:data => valid_parsed_data.to_json)).to be_dangerous
    end
  end

  describe "#import and #generate_diff" do
    let(:scenario_import) do
      _import = Huginn::ScenarioImport.new(:data => valid_data)
    ...
    context "when this scenario has never been seen before" do
      describe "#import" do
        ...
        it "creates new Agents, even if one already exists with the given guid (so that we don't overwrite a user's work outside of the scenario)" do
          huginn_agents(:bob_weather_agent).update_attribute :guid, "a-weather-agent"
      ...
      describe "#generate_diff" do
        it "returns AgentDiff objects for the incoming Agents" do
          ...
          valid_parsed_weather_agent_data.each do |key, value|
            ...
            expect(field).to be_a(Huginn::ScenarioImport::AgentDiff::FieldDiff)
    ...
    context "when an a scenario already exists with the given guid for the importing user" do
      let!(:existing_scenario) do
        ...
        huginn_agents(:bob_weather_agent).update_attribute :guid, "a-weather-agent"
        huginn_agents(:bob_weather_agent).scenarios << _existing_scenerio
      ...
      describe "#import" do
        ...
        it "updates any existing agents in the scenario, and makes new ones as needed" do
          ...
          expect(weather_agent).to eq(huginn_agents(:bob_weather_agent))
      ...
      describe "#generate_diff" do
        it "returns AgentDiff objects that include 'current' values from any agents that already exist" do
          ...
          # Already exists
          expect(weather_agent_diff.agent).to eq(huginn_agents(:bob_weather_agent))
          valid_parsed_weather_agent_data.each do |key, value|
            next if key == :type
            expect(weather_agent_diff.send(key).current).to eq(huginn_agents(:bob_weather_agent).send(key))
          end
        ...
        context "when the schema_version is less than 1" do
          it "translates keep_events_for from days to seconds" do
            ...
            expect(weather_agent_diff.name.current).to eq(huginn_agents(:bob_weather_agent).name)
    ...
    context "when Bob imports Jane's scenario" do
      ...
      describe "#import" do
        it "makes a new scenario for Bob" do
          ...
          expect(Huginn::Scenario.where(guid: guid).count).to eq(2)
    ...
    context "agents which require a service" do
      ...
      let(:services_scenario_import) {
        _import = Huginn::ScenarioImport.new(:data => valid_parsed_services_data)
        _import.set_user users(:bob)
        _import
      }
