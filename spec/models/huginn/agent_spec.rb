...
module Huginn
  describe Agent do
    ...
    describe '.active/inactive' do
      let(:agent) { huginn_agents(:jane_website_agent) }
    ...
    describe ".bulk_check" do
      ...
      it "should skip disabled Agents" do
        huginn_agents(:bob_weather_agent).update_attribute :disabled, true
      ...
      it "should skip agents of deactivated accounts" do
        huginn_agents(:bob_weather_agent).user.deactivate!
        mock(Agents::WeatherAgent).async_check(anything).times(@weather_agent_count - 1)
        Agents::WeatherAgent.bulk_check("midnight")
      end
    end
    ...
    describe ".run_schedule" do
      ...
      it "runs agents with the given schedule" do
        weather_agent_ids = [huginn_agents(:bob_weather_agent), huginn_agents(:jane_weather_agent)].map(&:id)
        stub(Agents::WeatherAgent).async_check(anything) {|agent_id| weather_agent_ids.delete(agent_id) }
        stub(Agents::WebsiteAgent).async_check(huginn_agents(:bob_website_agent).id)
      ...
      it "will not run the 'never' schedule" do
        huginn_agents(:bob_weather_agent).update_attribute 'schedule', 'never'
        do_not_allow(Agents::WebsiteAgent).async_check
    ...
    describe "credential" do
      it "should return the value of the credential when credential is present" do
        expect(huginn_agents(:bob_weather_agent).credential("aws_secret")).to eq(huginn_user_credentials(:bob_aws_secret).credential_value)
      end

      it "should return nil when credential is not present" do
        expect(huginn_agents(:bob_weather_agent).credential("non_existing_credential")).to eq(nil)
      end

      it "should memoize the load" do
        mock.any_instance_of(UserCredential).credential_value.twice { "foo" }
        expect(huginn_agents(:bob_weather_agent).credential("aws_secret")).to eq("foo")
        expect(huginn_agents(:bob_weather_agent).credential("aws_secret")).to eq("foo")
        huginn_agents(:bob_weather_agent).reload
        expect(huginn_agents(:bob_weather_agent).credential("aws_secret")).to eq("foo")
        expect(huginn_agents(:bob_weather_agent).credential("aws_secret")).to eq("foo")
      end
    end
    ...
    describe "changes to type" do
      it "validates types" do
        source = Agent.new
        source.type = "Huginn::Agents::WeatherAgent"
        expect(source).to have(0).errors_on(:type)
        source.type = "Huginn::Agents::WebsiteAgent"
        expect(source).to have(0).errors_on(:type)
        source.type = "Huginn::Agents::Fake"
        expect(source).to have(1).error_on(:type)
      end

      it "disallows changes to type once a record has been saved" do
        source = huginn_agents(:bob_website_agent)
        source.type = "Huginn::Agents::WeatherAgent"
        expect(source).to have(1).error_on(:type)
      end
    ...
    describe "with an example Agent" do
      ...
      before do
        stub(Agents::SomethingSource).valid_type?("Huginn::Agents::SomethingSource") { true }
        stub(Agents::CannotBeScheduled).valid_type?("Huginn::Agents::CannotBeScheduled") { true }
      end
      ...
      describe ".async_check" do
        ...
        it "should not run disabled Agents" do
          mock(Agent).find(huginn_agents(:bob_weather_agent).id) { huginn_agents(:bob_weather_agent) }
          do_not_allow(huginn_agents(:bob_weather_agent)).check
          huginn_agents(:bob_weather_agent).update_attribute :disabled, true
          Agent.async_check(huginn_agents(:bob_weather_agent).id)
        end
      end
      ...
      describe ".receive!" do
        ...
        it "should use available events" do
          Agent.async_check(huginn_agents(:bob_weather_agent).id)
          mock(Agent).async_receive(huginn_agents(:bob_rain_notifier_agent).id, anything).times(1)
          Agent.receive!
        end

        it "should not propogate to disabled Agents" do
          Agent.async_check(huginn_agents(:bob_weather_agent).id)
          huginn_agents(:bob_rain_notifier_agent).update_attribute :disabled, true
          mock(Agent).async_receive(huginn_agents(:bob_rain_notifier_agent).id, anything).times(0)
          Agent.receive!
        end
        ...
        it "should log exceptions" do
          ...
          Agent.async_check(huginn_agents(:bob_weather_agent).id)
          expect {
            Agent.async_receive(huginn_agents(:bob_rain_notifier_agent).id, [huginn_agents(:bob_weather_agent).events.last.id])
          }.to raise_error(RuntimeError)
          log = huginn_agents(:bob_rain_notifier_agent).logs.first
          expect(log.message).to match(/Exception/)
          expect(log.level).to eq(4)
        end
        ...
        it "should track when events have been seen and not received them again" do
          ...
          expect {
            Agent.receive!
          }.to change { huginn_agents(:bob_rain_notifier_agent).reload.last_checked_event_id }

          expect {
            Agent.receive!
          }.not_to change { huginn_agents(:bob_rain_notifier_agent).reload.last_checked_event_id }
        end
        ...
        it "should group events" do
          ...
          Agent.async_check(huginn_agents(:bob_weather_agent).id)
          Agent.async_check(huginn_agents(:jane_weather_agent).id)
          Agent.receive!
        end
        ...
        it "should ignore events that were created before a particular Link" do
          ...
          mock.any_instance_of(Agents::TriggerAgent).receive(anything).twice
          huginn_agents(:bob_weather_agent).check # bob_weather_agent makes an event
          ...
          # Now we link to it.
          huginn_agents(:bob_rain_notifier_agent).sources << agent2
      ...
      describe ".async_receive" do
        it "should not run disabled Agents" do
          mock(Agent).find(huginn_agents(:bob_rain_notifier_agent).id) { huginn_agents(:bob_rain_notifier_agent) }
          do_not_allow(huginn_agents(:bob_rain_notifier_agent)).receive
          huginn_agents(:bob_rain_notifier_agent).update_attribute :disabled, true
          Agent.async_receive(huginn_agents(:bob_rain_notifier_agent).id, [1, 2, 3])
        end
      end
      ...
      describe "validations" do
        ...
        it "should not allow source agents owned by other people" do
          agent = Agents::SomethingSource.new(:name => "something")
          agent.user = users(:bob)
          agent.source_ids = [huginn_agents(:bob_weather_agent).id]
          expect(agent).to have(0).errors_on(:sources)
          agent.source_ids = [huginn_agents(:jane_weather_agent).id]
        ...
        it "should not allow controller agents owned by other people" do
          agent = Agents::SomethingSource.new(:name => "something")
          agent.user = users(:bob)
          agent.controller_ids = [huginn_agents(:bob_weather_agent).id]
          expect(agent).to have(0).errors_on(:controllers)
          agent.controller_ids = [huginn_agents(:jane_weather_agent).id]
        ...
        it "should not allow control target agents owned by other people" do
          ...
          agent.user = users(:bob)
          agent.control_target_ids = [huginn_agents(:bob_weather_agent).id]
          expect(agent).to have(0).errors_on(:control_targets)
          agent.control_target_ids = [huginn_agents(:jane_weather_agent).id]
        ...
        it "should not allow scenarios owned by other people" do
          ...
          agent.scenario_ids = [huginn_scenarios(:bob_weather).id]
          expect(agent).to have(0).errors_on(:scenarios)

          agent.scenario_ids = [huginn_scenarios(:bob_weather).id, huginn_scenarios(:jane_weather).id]
          expect(agent).to have(1).errors_on(:scenarios)

          agent.scenario_ids = [huginn_scenarios(:jane_weather).id]
          expect(agent).to have(1).errors_on(:scenarios)
    ...
    describe ".trigger_web_request" do
      ...
      before do
        stub(Agents::WebRequestReceiver).valid_type?("Huginn::Agents::WebRequestReceiver") { true }
      end
    ...
    describe "scopes" do
      describe "of_type" do
        it "should accept classes" do
          ...
          expect(agents).to include(huginn_agents(:bob_website_agent))
          expect(agents).to include(huginn_agents(:jane_website_agent))
          expect(agents).not_to include(huginn_agents(:bob_weather_agent))
        end

        it "should accept strings" do
          agents = Agent.of_type("Huginn::Agents::WebsiteAgent")
          expect(agents).to include(huginn_agents(:bob_website_agent))
          expect(agents).to include(huginn_agents(:jane_website_agent))
          expect(agents).not_to include(huginn_agents(:bob_weather_agent))
        end
        ...
        it "should accept instances of an Agent" do
          agents = Agent.of_type(huginn_agents(:bob_website_agent))
          expect(agents).to include(huginn_agents(:bob_website_agent))
          expect(agents).to include(huginn_agents(:jane_website_agent))
          expect(agents).not_to include(huginn_agents(:bob_weather_agent))
        end
    ...
    describe "#create_event" do
      describe "when the agent has keep_events_for set" do
        before do
          expect(huginn_agents(:jane_weather_agent).keep_events_for).to be > 0
        end

        it "sets expires_at on created events" do
          event = huginn_agents(:jane_weather_agent).create_event :payload => { 'hi' => 'there' }
          expect(event.expires_at.to_i).to be_within(5).of(huginn_agents(:jane_weather_agent).keep_events_for.seconds.from_now.to_i)
        end
      end

      describe "when the agent does not have keep_events_for set" do
        before do
          expect(huginn_agents(:jane_website_agent).keep_events_for).to eq(0)
        end

        it "does not set expires_at on created events" do
          event = huginn_agents(:jane_website_agent).create_event :payload => { 'hi' => 'there' }
    ...
    describe '.last_checked_event_id' do
      it "should be updated by setting drop_pending_events to true" do
        agent = huginn_agents(:bob_rain_notifier_agent)
    ...
    describe ".drop_pending_events" do
      ...
      it "should drop pending events while the agent was disabled when set to true" do
        agent1 = huginn_agents(:bob_weather_agent)
        agent2 = huginn_agents(:bob_rain_notifier_agent)
