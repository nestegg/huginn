...
describe Huginn::Agents::EventFormattingAgent do
  before do
    @valid_params = {
        :name => "somename",
        :options => {
    ...
    @checker = Huginn::Agents::EventFormattingAgent.new(@valid_params)
    ...
    @event2.agent = huginn_agents(:jane_weather_agent)
  ...
  describe "#receive" do
    it "should accept clean mode" do
      ...
      expect(Huginn::Event.last.payload[:content]).to eq(nil)
    end

    it "should accept merge mode" do
      @checker.options[:mode] = "merge"
      @checker.receive([@event])
      expect(Huginn::Event.last.payload[:content]).not_to eq(nil)
    end

    it "should handle Liquid templating in instructions" do
      @checker.receive([@event])
      expect(Huginn::Event.last.payload[:message]).to eq("Received Some Lorem Ipsum from somevalue .")
      expect(Huginn::Event.last.payload[:agent]).to eq("WeatherAgent")
      expect(Huginn::Event.last.payload[:created_at]).to eq(@event.created_at.to_s)
      expect(Huginn::Event.last.payload[:created_at_iso]).to eq(@event.created_at.iso8601)
    end
    ...
    it "should handle matchers and Liquid templating in instructions" do
      ...
      formatted_event1, formatted_event2 = Huginn::Event.last(2)
    ...
    it "should allow escaping" do
      ...
      @checker.save!
      @checker.receive([@event])
      expect(Huginn::Event.last.payload[:message]).to eq("Escaped: escape+this%21%3F\nNot escaped: escape this!?")
    end

    it "should handle multiple events" do
      event1 = Huginn::Event.new
      event1.agent = huginn_agents(:bob_weather_agent)
      event1.payload = {
          :content => {
              :text => "Some Lorem Ipsum",
              :name => "somevalue"
