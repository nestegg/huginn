...
describe Huginn::Agents::LocalFileAgent do
  before(:each) do
    @valid_params = {
                      'mode' => 'read',
                      'watch' => 'false',
    ...
    @checker = Huginn::Agents::LocalFileAgent.new(:name => "somename", :options => @valid_params)
  ...
  context "#check_path_existance" do
    ...
    it "create a log entry" do
      @checker.options['path'] = '/doesnotexist'
      expect { @checker.check_path_existance(true) }.to change(Huginn::AgentLog, :count).by(1)
    end
  ...
  context "#check" do
    ...
    it "creates an event for every file in the directory" do
      with_files(File.join(Rails.root, 'tmp', 'spec', 'one'), File.join(Rails.root, 'tmp', 'spec', 'two')) do
        ...
        expect(Huginn::Event.last.payload.has_key?('file_pointer')).to be_truthy
      end
    end
    ...
    it "creates an event if the configured file exists" do
      ...
      with_files(File.join(Rails.root, 'tmp', 'spec', 'one'), File.join(Rails.root, 'tmp', 'spec', 'two')) do
        ...
        payload = Huginn::Event.last.payload
  ...
  context "#receive" do
    ...
    it "does not receive when ENABLE_INSECURE_AGENTS is not set to true" do
      ENV['ENABLE_INSECURE_AGENTS'] = 'false'
      expect { @checker.receive([]) }.to change(Huginn::AgentLog, :count).by(1)
      ENV['ENABLE_INSECURE_AGENTS'] = 'true'
    end
  end

  describe describe Huginn::Agents::LocalFileAgent::Worker do
    require 'listen'

    before(:each) do
      @checker.options['watch'] = true
      @checker.save
      @worker = Huginn::Agents::LocalFileAgent::Worker.new(agent: @checker)
      @listen_mock = mock()
    end
    ...
    context "#callback" do
      ...
      it "creates an event for modifies files" do
        expect { @worker.send(:callback, [file], [], [])}.to change(Huginn::Event, :count).by(1)
        payload = Huginn::Event.last.payload
      ...
      it "creates an event each changed file" do
        expect { @worker.send(:callback, [], [file], [file2])}.to change(Huginn::Event, :count).by(2)
      end
    end
