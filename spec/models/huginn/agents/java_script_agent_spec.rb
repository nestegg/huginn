...
describe Huginn::Agents::JavaScriptAgent do
  before do
    @valid_params = {
      :name => "somename",
      :options => {
    ...
    @agent = Huginn::Agents::JavaScriptAgent.new(@valid_params)
  ...
  describe "#working?" do
    ...
    describe "when expected_receive_period_in_days is set" do
      it "returns false when more than expected_receive_period_in_days have passed since the last event was received" do
        ...
        @agent.save!
        expect(@agent).not_to be_working
        Huginn::Agents::JavaScriptAgent.async_receive @agent.id, [huginn_events(:bob_website_agent_event).id]
        expect(@agent.reload).to be_working
  ...
  describe "executing code" do
    it "works by default" do
      ...
      expect {
        expect {
          @agent.receive([huginn_events(:bob_website_agent_event)])
          @agent.check
        }.not_to change { Huginn::AgentLog.count }
    ...
    describe "using credentials as code" do
      ...
      it "accepts credentials" do
        @agent.check
        expect(Huginn::AgentLog.last.message).to eq("ran it")
      end

      it "logs an error when the credential goes away" do
        @agent.user.user_credentials.delete_all
        @agent.reload.check
        expect(Huginn::AgentLog.last.message).to eq("Unable to find credential")
      end
    end
    ...
    describe "error handling" do
      it "should log an error when V8 has issues" do
        ...
        }.to change { Huginn::AgentLog.count }.by(1)
        expect(Huginn::AgentLog.last.message).to match(/Unexpected identifier/)
        expect(Huginn::AgentLog.last.level).to eq(4)
      end
      ...
      it "should log an error when JavaScript throws" do
        ...
        expect(Huginn::AgentLog.last.message).to match(/oh no/)
        expect(Huginn::AgentLog.last.level).to eq(4)
      end
    ...
    describe "logging" do
      it "can output AgentLogs with this.log and this.error in the JavaScript environment" do
        ...
        }.to change { Huginn::AgentLog.count }.by(2)

        log1, log2 = Huginn::AgentLog.last(2)
    ...
    describe "getting incoming events" do
      it "can access incoming events in the JavaScript enviroment via this.incomingEvents" do
        ...
        expect {
          expect {
            @agent.receive([huginn_events(:bob_website_agent_event), event])
          }.not_to change { Huginn::AgentLog.count }
        }.to change { Huginn::Event.count }.by(2)
    ...
    describe "using CoffeeScript" do
      it "will accept a 'language' of 'CoffeeScript'" do
        ...
        expect(Huginn::AgentLog.last.message).to eq("hello from coffeescript")
      end
    end
    ...
    describe "user credentials" do
      it "can access an existing credential" do
        ...
        @agent.save!
        @agent.check
        expect(Huginn::AgentLog.last.message).to eq("hello")
      end
      ...
      it "will create a new credential" do
        ...
        expect {
          @agent.check
        }.to change(Huginn::UserCredential, :count).by(1)
      end
      ...
      it "updates an existing credential" do
        ...
        expect {
          @agent.check
        }.to change(Huginn::UserCredential, :count).by(0)
