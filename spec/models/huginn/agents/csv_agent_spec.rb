...
describe Huginn::Agents::CsvAgent do
  before(:each) do
    @valid_params = {
                      'mode' => 'parse',
                      'separator' => ',',
    ...
    @checker = Huginn::Agents::CsvAgent.new(:name => 'somename', :options => @valid_params)
    @checker.user = users(:jane)
    @checker.save!
    @lfa = Huginn::Agents::LocalFileAgent.new(name: 'local', options: {path: '{{}}', watch: 'false', append: 'false', mode: 'read'})
    @lfa.user = users(:jane)
    @lfa.save!
  end
  ...
  context '#receive' do
    ...
    def event_with_contents(contents)
      ...
      Huginn::Event.new(payload: { 'file_pointer' => {'agent_id' => @lfa.id, 'file' => path } }, user_id: @checker.user_id)
    end
    ...
    context "agent options" do
      ...
      context "output" do
        it "creates one event per row" do
          ...
          expect { @checker.receive([with_headers]) }.to change(Huginn::Event, :count).by(2)
          expect(Huginn::Event.last.payload).to eq(@checker.options['data_key'] => {'one' => '2', 'two' => '3'})
        end

        it "creates one event per file" do
          @checker.options['output'] = 'event_per_file'
          expect { @checker.receive([with_headers]) }.to change(Huginn::Event, :count).by(1)
          expect(Huginn::Event.last.payload).to eq(@checker.options['data_key'] => [{"one"=>"1", "two"=>"2"}, {"one"=>"2", "two"=>"3"}])
        end
      end
      ...
      context "with_header" do
        it "works without headers" do
          @checker.options['with_header'] = 'false'
          expect { @checker.receive([without_headers]) }.to change(Huginn::Event, :count).by(2)
          expect(Huginn::Event.last.payload).to eq({@checker.options['data_key']=>["2", "3"]})
        end

        it "works without headers and event_per_file" do
          @checker.options['with_header'] = 'false'
          @checker.options['output'] = 'event_per_file'
          expect { @checker.receive([without_headers]) }.to change(Huginn::Event, :count).by(1)
          expect(Huginn::Event.last.payload).to eq({@checker.options['data_key']=>[['1', '2'], ["2", "3"]]})
        end
      end
      ...
      context "use_fields" do
        it "extracts the specified columns" do
          ...
          expect { @checker.receive([with_headers]) }.to change(Huginn::Event, :count).by(2)
          expect(Huginn::Event.last.payload).to eq(@checker.options['data_key'] => {'one' => '2'})
        end
      end
      ...
      context "data_path" do
        it "can receive the CSV via a regular event" do
          ...
          event = Huginn::Event.new(payload: {'data' => "one,two\r\n1,2\r\n2,3"})
          expect { @checker.receive([event]) }.to change(Huginn::Event, :count).by(2)
          expect(Huginn::Event.last.payload).to eq(@checker.options['data_key'] => {'one' => '2', 'two' => '3'})
        end
      end
    end
    ...
    context "handling different CSV formats" do
      ...
      it "handles quotes correctly" do
        event = event_with_contents("\"one\",\"two\"\n1,2\n\"\"2, two\",3")
        expect { @checker.receive([event]) }.to change(Huginn::Event, :count).by(2)
        expect(Huginn::Event.last.payload).to eq(@checker.options['data_key'] => {'one' => '"2, two', 'two' => '3'})
      end
    ...
    context "serializing" do
      ...
      it "writes headers when with_header is true" do
        event = Huginn::Event.new(payload: { 'data' => {'key' => 'value', 'key2' => 'value2', 'key3' => 'value3'} })
        expect { @checker.receive([event])}.to change(Huginn::Event, :count).by(1)
        expect(Huginn::Event.last.payload).to eq('data' => "\"key\",\"key2\",\"key3\"\n\"value\",\"value2\",\"value3\"\n")
      end

      it "writes one row per received event" do
        event = Huginn::Event.new(payload: { 'data' => {'key' => 'value', 'key2' => 'value2', 'key3' => 'value3'} })
        event2 = Huginn::Event.new(payload: { 'data' => {'key' => '2value', 'key2' => '2value2', 'key3' => '2value3'} })
        expect { @checker.receive([event, event2])}.to change(Huginn::Event, :count).by(1)
        expect(Huginn::Event.last.payload).to eq('data' => "\"key\",\"key2\",\"key3\"\n\"value\",\"value2\",\"value3\"\n\"2value\",\"2value2\",\"2value3\"\n")
      end

      it "accepts multiple rows per event" do
        event = Huginn::Event.new(payload: { 'data' => [{'key' => 'value', 'key2' => 'value2', 'key3' => 'value3'}, {'key' => '2value', 'key2' => '2value2', 'key3' => '2value3'}] })
        expect { @checker.receive([event])}.to change(Huginn::Event, :count).by(1)
        expect(Huginn::Event.last.payload).to eq('data' => "\"key\",\"key2\",\"key3\"\n\"value\",\"value2\",\"value3\"\n\"2value\",\"2value2\",\"2value3\"\n")
      end
      ...
      it "does not write the headers when with_header is false" do
        ...
        event = Huginn::Event.new(payload: { 'data' => {'key' => 'value', 'key2' => 'value2', 'key3' => 'value3'} })
        expect { @checker.receive([event])}.to change(Huginn::Event, :count).by(1)
        expect(Huginn::Event.last.payload).to eq('data' => "\"value\",\"value2\",\"value3\"\n")
      end
      ...
      it "only serialize the keys specified in use_fields" do
        ...
        event = Huginn::Event.new(payload: { 'data' => {'key' => 'value', 'key2' => 'value2', 'key3' => 'value3'} })
        expect { @checker.receive([event])}.to change(Huginn::Event, :count).by(1)
        expect(Huginn::Event.last.payload).to eq('data' => "\"key2\",\"key3\"\n\"value2\",\"value3\"\n")
      end
      ...
      it "respects the order of use_fields" do
        ...
        event = Huginn::Event.new(payload: { 'data' => {'key' => 'value', 'key2' => 'value2', 'key3' => 'value3'} })
        expect { @checker.receive([event])}.to change(Huginn::Event, :count).by(1)
        expect(Huginn::Event.last.payload).to eq('data' => "\"key3\",\"key\"\n\"value3\",\"value\"\n")
      end
      ...
      it "respects use_fields and writes no header" do
        ...
        event = Huginn::Event.new(payload: { 'data' => {'key' => 'value', 'key2' => 'value2', 'key3' => 'value3'} })
        expect { @checker.receive([event])}.to change(Huginn::Event, :count).by(1)
        expect(Huginn::Event.last.payload).to eq('data' => "\"value2\",\"value3\"\n")
      end

      context "arrays" do
        it "does not write a header" do
          @checker.options['with_header'] = 'false'
          event = Huginn::Event.new(payload: { 'data' => ['value1', 'value2'] })
          event2 = Huginn::Event.new(payload: { 'data' => ['value3', 'value4'] })
          expect { @checker.receive([event, event2])}.to change(Huginn::Event, :count).by(1)
          expect(Huginn::Event.last.payload).to eq('data' => "\"value1\",\"value2\"\n\"value3\",\"value4\"\n")
        end

        it "handles nested arrays" do
          event = Huginn::Event.new(payload: { 'data' => [['value1', 'value2'], ['value3', 'value4']] })
          expect { @checker.receive([event])}.to change(Huginn::Event, :count).by(1)
          expect(Huginn::Event.last.payload).to eq('data' => "\"value1\",\"value2\"\n\"value3\",\"value4\"\n")
        end
      end
