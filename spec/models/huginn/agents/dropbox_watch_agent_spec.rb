...
describe Huginn::Agents::DropboxWatchAgent do
  before(:each) do
    @agent = Huginn::Agents::DropboxWatchAgent.new(
  ...
  describe '#check' do
    ...
    context 'first time' do
      ...
      it 'does not send any events' do
        expect { @agent.check }.to_not change(Huginn::Event, :count)
      end
    ...
    context 'subsequent calls' do
      ...
      it 'sends an event upon a different directory listing' do
        ...
        stub.proxy(Huginn::Agents::DropboxWatchAgent::DropboxDirDiff).new(@agent.memory['contents'], second_result) do |diff|
          stub(diff).empty? { false }
          stub(diff).to_hash { payload }
        end
        expect { @agent.check }.to change(Huginn::Event, :count).by(1)
        expect(Huginn::Event.last.payload).to eq(payload)
      end
  ...
  describe Huginn::Agents::DropboxWatchAgent::DropboxDirDiff do
    ...
    describe '#empty?' do
      ...
      it 'is true when no differences are detected' do
        diff = Huginn::Agents::DropboxWatchAgent::DropboxDirDiff.new(previous, previous)
        expect(diff.empty?).to eq true
      end

      it 'is false when differences were detected' do
        diff = Huginn::Agents::DropboxWatchAgent::DropboxDirDiff.new(previous, current)
    ...
    describe '#to_hash' do
      ...
      subject(:diff_hash) { Huginn::Agents::DropboxWatchAgent::DropboxDirDiff.new(previous, current).to_hash }
      ...
      context 'when the previous value is not defined' do
        it 'considers all additions' do
          diff_hash = Huginn::Agents::DropboxWatchAgent::DropboxDirDiff.new(nil, current).to_hash
      ...
      context 'when the current value is not defined' do
        it 'considers all removals' do
          diff_hash = Huginn::Agents::DropboxWatchAgent::DropboxDirDiff.new(previous, nil).to_hash
