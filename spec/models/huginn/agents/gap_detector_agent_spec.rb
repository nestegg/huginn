...
describe Huginn::Agents::GapDetectorAgent do
  let(:valid_params) {
    {
  ...
  let(:agent) {
    _agent = Huginn::Agents::GapDetectorAgent.new(valid_params)
  ...
  describe '#receive' do
    it 'records the event if it has a created_at newer than the last seen' do
      ...
      expect(agent.memory['newest_event_created_at']).to eq huginn_events(:bob_website_agent_event).created_at.to_i

      huginn_events(:bob_website_agent_event).created_at = 2.days.ago

      expect {
        agent.receive([huginn_events(:bob_website_agent_event)])
      }.to_not change { agent.memory['newest_event_created_at'] }

      huginn_events(:bob_website_agent_event).created_at = 2.days.from_now

      expect {
        agent.receive([huginn_events(:bob_website_agent_event)])
      }.to change { agent.memory['newest_event_created_at'] }.to(huginn_events(:bob_website_agent_event).created_at.to_i)
    end
    ...
    it 'ignores the event if value_path is present and the value at the path is blank' do
      ...
      huginn_events(:bob_website_agent_event).created_at = 2.days.from_now
      huginn_events(:bob_website_agent_event).payload['title'] = ''

      expect {
        agent.receive([huginn_events(:bob_website_agent_event)])
      }.to_not change { agent.memory['newest_event_created_at'] }

      huginn_events(:bob_website_agent_event).payload['title'] = 'present!'
