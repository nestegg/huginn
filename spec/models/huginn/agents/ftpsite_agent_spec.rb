...
describe Huginn::Agents::FtpsiteAgent do
  describe "checking anonymous FTP" do
    before do
      ...
      @checker = Huginn::Agents::FtpsiteAgent.new(:name => "Example", :options => @site, :keep_events_for => 2.days)
    ...
    context "#validate_options" do
      ...
      it 'automatically sets mode to read when the agent is a new record' do
        checker = Huginn::Agents::FtpsiteAgent.new(name: 'test', options: @site.except('mode'))
        checker.user = users(:bob)
        expect(checker).to be_valid
    ...
    describe "#check" do
      ...
      it "should check for changes and save known entries in memory" do
        expect { @checker.check }.to change { Huginn::Event.count }.by(3)
        ...
        expect(Huginn::Event.last(2).first.payload).to eq({
