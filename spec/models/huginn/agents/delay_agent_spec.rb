...
describe Huginn::Agents::DelayAgent do
  let(:agent) do
    _agent = Huginn::Agents::DelayAgent.new(name: 'My DelayAgent')
  ...
  def create_event
    _event = Huginn::Event.new(payload: { random: rand })
    _event.agent = huginn_agents(:bob_website_agent)
    _event.save!
    _event
  end
  ...
  describe "#working?" do
    it "checks if events have been received within expected receive period" do
      ...
      Huginn::Agents::DelayAgent.async_receive agent.id, [huginn_events(:bob_website_agent_event).id]
