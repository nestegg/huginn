...
describe Huginn::Agents::EvernoteAgent do
  ...
  before do
    stub.any_instance_of(Huginn::Agents::EvernoteAgent).evernote_note_store { en_note_store }
  end
  ...
  describe "#receive" do
    context "when mode is set to 'update'" do
      before do
        ...
        @agent = Huginn::Agents::EvernoteAgent.new(:name => "evernote updater", :options => @options)
        @agent.service = huginn_services(:generic)
        @agent.user = users(:bob)
        @agent.save!

        @event = Huginn::Event.new
        @event.agent = huginn_agents(:bob_website_agent)
      ...
      it "adds a note for any payload it receives" do
        ...
        Huginn::Agents::EvernoteAgent.async_receive(@agent.id, [@event.id])
  ...
  describe "#check" do
    context "when mode is set to 'read'" do
      before do
        ...
        @checker = Huginn::Agents::EvernoteAgent.new(:name => "evernote reader", :options => @options)
      ...
      context "the first time it checks" do
        it "returns only notes created/updated since it was created" do
          expect { @checker.check }.to change { Huginn::Event.count }.by(0)
        end
      end
  ...
  # api wrapper classes
  describe Huginn::Agents::EvernoteAgent::NoteStore do
    let(:note_store) { Huginn::Agents::EvernoteAgent::NoteStore.new(en_note_store) }
    ...
    describe "#create_note" do
      ...
      it "returns a note" do
        expect(note_store.create_note(title: "third note")).to be_a(Huginn::Agents::EvernoteAgent::Note)
      end
    end
    ...
    describe "#update_note" do
      ...
      it "returns a note" do
        expect(note_store.update_note(guid: 1, content: "some words")).to be_a(Huginn::Agents::EvernoteAgent::Note)
      end
    end
    ...
    describe "#find_note" do
      it "gets a note with the given guid" do
        ...
        expect(note).to be_a(Huginn::Agents::EvernoteAgent::Note)
      end
    end
    ...
    describe "#create_or_update_note" do
      ...
      context "a note with given title and notebook does not exist" do
        ...
        it "creates a note" do
          ...
          expect(result).to be_a(Huginn::Agents::EvernoteAgent::Note)
  ...
  describe Huginn::Agents::EvernoteAgent::NoteStore::Search do
    let(:note_store) { Huginn::Agents::EvernoteAgent::NoteStore.new(en_note_store) }
    ...
    let(:search) do
      Huginn::Agents::EvernoteAgent::NoteStore::Search.new(note_store,
    ...
    describe "#notes" do
      context "last_checked_at is not set" do
        it "returns notes satisfying the search options" do
          ...
          expect(result.first).to be_a(Huginn::Agents::EvernoteAgent::Note)
        end
      end
  ...
  describe Huginn::Agents::EvernoteAgent::Note do
    let(:resource) {
    ...
    describe "#attr" do
      let(:note) {
        Huginn::Agents::EvernoteAgent::Note.new(en_note, "xkcd", ["funny", "data"])
      }
