...
describe Huginn::Agents::WitaiAgent do
  before do
    ...
    @checker = Huginn::Agents::WitaiAgent.new :name => 'wit.ai agent',
  ...
  describe '#working' do
    it 'checks if agent is working when event is received withing expected number of days' do
      ...
      Huginn::Agents::WitaiAgent.async_receive @checker.id, [@event.id]
  ...
  describe '#receive' do
    ...
    it 'checks the integrity of new event' do
      @checker.receive([@event])
      expect(Huginn::Event.last.payload[:outcomes][0][:_text]).to eq(@event.payload[:message][:content])
    end
  end
end
