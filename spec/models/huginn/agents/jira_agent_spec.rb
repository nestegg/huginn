...
describe Huginn::Agents::JiraAgent do
  before(:each) do
    ...
    @checker = Huginn::Agents::JiraAgent.new(:name => "jira-agent", :options => @valid_params)
  ...
  describe "#check" do
    it "should be able to retrieve issues" do
      ...
      expect { @checker.check }.to change { Huginn::Event.count }.by(50)
    end
  end
