...
describe Huginn::Agents::GoogleCalendarPublishAgent, :vcr do
  let(:valid_params) do
    {
  ...
  let(:agent) do
    _agent = Huginn::Agents::GoogleCalendarPublishAgent.new(name: "somename", options: valid_params)
  ...
  describe '#receive' do
    let(:event) do
      _event = Huginn::Event.new
      _event.agent = huginn_agents(:bob_manual_event_agent)
