...
describe Huginn::Agents::HumanTaskAgent do
  before do
    @checker = Huginn::Agents::HumanTaskAgent.new(:name => "my human task agent")
  ...
  describe "when 'trigger_on' is set to 'schedule'" do
    ...
    it "should ignore events" do
      ...
      @checker.receive([huginn_events(:bob_website_agent_event)])
    end
  end
  ...
  describe "when 'trigger_on' is set to 'event'" do
    ...
    it "should create HITs based on events" do
      mock(@checker).create_basic_hit(huginn_events(:bob_website_agent_event)).times(1)
      @checker.receive([huginn_events(:bob_website_agent_event)])
    end
  end
  ...
  describe "creating hits" do
    it "can create HITs based on events, interpolating their values" do
      ...
      hitInterface.id = 123
      mock(hitInterface).question_form(instance_of Huginn::Agents::HumanTaskAgent::AgentQuestionForm) { |agent_question_form_instance| question_form = agent_question_form_instance }
    ...
    it "works without an event too" do
      ...
      hitInterface = OpenStruct.new
      hitInterface.id = 123
      mock(hitInterface).question_form(instance_of Huginn::Agents::HumanTaskAgent::AgentQuestionForm)
  ...
  describe "reviewing HITs" do
    ...
    describe "creating and reviewing polls" do
      ...
      it "creates a poll using the row_template, message, and correct number of assignments" do
        ...
        expect {
          @checker.send :review_hits
        }.to change { Huginn::Event.count }.by(0) # it does not emit an event until all poll results are in
