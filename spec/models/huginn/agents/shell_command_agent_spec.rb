...
describe Huginn::Agents::ShellCommandAgent do
  before do
    @valid_path = Dir.pwd
    ...
    @checker = Huginn::Agents::ShellCommandAgent.new(name: 'somename', options: @valid_params)
    @checker.user = users(:jane)
    @checker.save!

    @checker2 = Huginn::Agents::ShellCommandAgent.new(name: 'somename2', options: @valid_params2)
    ...
    stub(Huginn::Agents::ShellCommandAgent).should_run? { true }
  end
  ...
  describe "#check" do
    ...
    it "should create an event when checking" do
      ...
      expect(Huginn::Event.last.payload[:path]).to eq(@valid_path)
      expect(Huginn::Event.last.payload[:command]).to eq('pwd')
      expect(Huginn::Event.last.payload[:output]).to eq("fake pwd output")
    end

    it "should create an event when checking (unstubbed)" do
      expect { @checker2.check }.to change { Huginn::Event.count }.by(1)
      expect(Huginn::Event.last.payload[:path]).to eq(@valid_path)
      expect(Huginn::Event.last.payload[:command]).to eq([RbConfig.ruby, '-e', 'puts "hello, #{STDIN.eof? ? "world" : STDIN.read.strip}."; STDERR.puts "warning!"'])
      expect(Huginn::Event.last.payload[:output]).to eq('hello, world.')
      expect(Huginn::Event.last.payload[:errors]).to eq('warning!')
    end
    ...
    it "does not run when should_run? is false" do
      stub(Huginn::Agents::ShellCommandAgent).should_run? { false }
      expect { @checker.check }.not_to change { Huginn::Event.count }
    end
  end
  ...
  describe "#receive" do
    ...
    it "creates events" do
      ...
      expect(Huginn::Event.last.payload[:path]).to eq(@valid_path)
      expect(Huginn::Event.last.payload[:command]).to eq(@event.payload[:cmd])
      expect(Huginn::Event.last.payload[:output]).to eq("fake ls output")
    end

    it "creates events (unstubbed)" do
      @checker2.receive([@event])
      expect(Huginn::Event.last.payload[:path]).to eq(@valid_path)
      expect(Huginn::Event.last.payload[:output]).to eq('hello, Huginn.')
      expect(Huginn::Event.last.payload[:errors]).to eq('warning!')
    end
