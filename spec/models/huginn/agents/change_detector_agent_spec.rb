...
describe Huginn::Agents::ChangeDetectorAgent do
  ...
  before do
    ...
    @checker = Huginn::Agents::ChangeDetectorAgent.new(:name => "somename", :options => @valid_params)
  ...
  describe "#receive" do
    ...
    it "creates events when memory is empty" do
      ...
      expect(Huginn::Event.last.payload[:command]).to eq(@event.payload[:command])
      expect(Huginn::Event.last.payload[:output]).to eq(@event.payload[:output])
    end
