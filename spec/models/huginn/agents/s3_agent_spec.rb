...
describe Huginn::Agents::S3Agent do
  before(:each) do
    @valid_params = {
                      'mode' => 'read',
    ...
    @checker = Huginn::Agents::S3Agent.new(:name => "somename", :options => @valid_params)
  ...
  context "#check" do
    context "not watching" do
      it "emits an event for every file" do
        ...
        expect { @checker.check }.to change(Huginn::Event, :count).by(2)
        expect(Huginn::Event.last.payload).to eq({"file_pointer" => {"file"=>"test2", "agent_id"=> @checker.id}})
      end
    end
    ...
    context "watching" do
      ...
      context "detecting changes" do
        ...
        it "emits events for removed files" do
          ...
          expect(Huginn::Event.last.payload).to eq({"file_pointer" => {"file" => "test2", "agent_id"=> @checker.id}, "event_type" => "removed"})
        end
        ...
        it "emits events for modified files" do
          ...
          mock(@checker).get_bucket_contents { contents }
          expect { @checker.check }.to change(Huginn::Event, :count).by(1)
          expect(Huginn::Event.last.payload).to eq({"file_pointer" => {"file" => "test2", "agent_id"=> @checker.id}, "event_type" => "modified"})
        end
        it "emits events for added files" do
          contents = {"test"=>"231232", "test2"=>"4564545", "test3" => "31231231"}
          mock(@checker).get_bucket_contents { contents }
          expect { @checker.check }.to change(Huginn::Event, :count).by(1)
          expect(Huginn::Event.last.payload).to eq({"file_pointer" => {"file" => "test3", "agent_id"=> @checker.id}, "event_type" => "added"})
        end
      end
      ...
      context "error handling" do
        it "handles AccessDenied exceptions" do
          ...
          expect { @checker.check }.to change(Huginn::AgentLog, :count).by(1)
          expect(Huginn::AgentLog.last.message).to eq("Could not access 'testbucket' Aws::S3::Errors::AccessDenied ")
        end

        it "handles generic S3 exceptions" do
          mock(@checker).get_bucket_contents { raise Aws::S3::Errors::PermanentRedirect.new('', 'error') }
          expect { @checker.check }.to change(Huginn::AgentLog, :count).by(1)
          expect(Huginn::AgentLog.last.message).to eq("Aws::S3::Errors::PermanentRedirect: error")
        end
      end
    end
  end
  ...
  context "#receive" do
    ...
    it "writes the data at data into a file" do
      ...
      event = Huginn::Event.new(payload: {'data' => 'hello world!'})
