...
describe Huginn::Agents::TelegramAgent do
  before do
    default_options = {
      auth_token: 'xxxxxxxxx:xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
      chat_id: 'xxxxxxxx'
    }
    @checker = Huginn::Agents::TelegramAgent.new name: 'Telegram Tester', options: default_options
  ...
  def stub_methods
    stub.any_instance_of(Huginn::Agents::TelegramAgent).send_telegram_message do |method, params|
      @sent_messages << { method => params }
    end

    stub.any_instance_of(Huginn::Agents::TelegramAgent).load_file do |_url|
      :stubbed_file
    end
  end
