...
describe Huginn::Agents::JsonParseAgent do
  before(:each) do
    @checker = Huginn::Agents::JsonParseAgent.new(:name => "somename", :options => Huginn::Agents::JsonParseAgent.new.default_options)
  ...
  describe "#receive" do
    it "parses valid JSON" do
      event = Huginn::Event.new(payload: { data: '{"test": "data"}' } )
    ...
    it "writes to the error log when the JSON could not be parsed" do
      event = Huginn::Event.new(payload: { data: '{"test": "data}' } )
      expect { @checker.receive([event]) }.to change(Huginn::AgentLog, :count).by(1)
    end
  end
end
