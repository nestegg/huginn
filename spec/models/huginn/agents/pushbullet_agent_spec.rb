...
describe Huginn::Agents::PushbulletAgent do
  before(:each) do
    @valid_params = {
                      'api_key'   => 'token',
                      'device_id' => '124',
    ...
    @checker = Huginn::Agents::PushbulletAgent.new(:name => "somename", :options => @valid_params)
  ...
  describe '#validate_api_key' do
    ...
    it "should return true when working" do
      mock(@checker).devices { raise Huginn::Agents::PushbulletAgent::Unauthorized }
  ...
  describe '#devices' do
    ...
    it "should return an empty array on error" do
      stub(@checker).request { raise Huginn::Agents::PushbulletAgent::Unauthorized }
