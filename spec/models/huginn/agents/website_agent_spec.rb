...
describe Huginn::Agents::WebsiteAgent do
  describe "checking without basic auth" do
    before do
      ...
      @checker = Huginn::Agents::WebsiteAgent.new(:name => "xkcd", :options => @valid_options, :keep_events_for => 2.days)
      @checker.user = users(:bob)
      @checker.save!
    end

    it_behaves_like Huginn::WebRequestConcern
    ...
    describe "#check" do
      it "should check for changes (and update Event.expires_at)" do
        expect { @checker.check }.to change { Huginn::Event.count }.by(1)
        event = Huginn::Event.last
        sleep 2
        expect { @checker.check }.not_to change { Huginn::Event.count }
        update_event = Huginn::Event.last
      ...
      it "should always save events when in :all mode" do
        expect {
          ...
          @checker.check
          @checker.check
        }.to change { Huginn::Event.count }.by(2)
      end
      ...
      it "should take uniqueness_look_back into account during deduplication" do
        ...
        expect {
          ...
          @checker.options = @valid_options
          @checker.check
        }.not_to change { Huginn::Event.count }
    ...
    describe 'http_success_codes' do
      it 'should allow scraping from a 404 result' do
        ...
        checker = Huginn::Agents::WebsiteAgent.new(:name => "Weather Site", :options => site)
        checker.user = users(:bob)
        checker.save!
    ...
    describe 'unzipping' do
      ...
      it 'should either avoid or support a raw deflate stream (#1018)' do
        ...
        checker = Huginn::Agents::WebsiteAgent.new(name: "Deflate Test", options: site)
        checker.user = users(:bob)
        checker.save!
    ...
    describe 'encoding' do
      it 'should be forced with force_encoding option' do
        ...
        checker = Huginn::Agents::WebsiteAgent.new(name: "No Encoding Site", options: site)
        checker.user = users(:bob)
        checker.save!
      ...
      it 'should be overridden with force_encoding option' do
        ...
        checker = Huginn::Agents::WebsiteAgent.new(name: "Wrong Encoding Site", options: site)
        checker.user = users(:bob)
        checker.save!
      ...
      it 'should be determined by charset in Content-Type' do
        ...
        checker = Huginn::Agents::WebsiteAgent.new(name: "Charset reader", options: site)
        checker.user = users(:bob)
        checker.save!
    ...
    describe "parsing" do
      ...
      it "should turn relative urls to absolute" do
        ...
        rel = Huginn::Agents::WebsiteAgent.new(:name => "xkcd", :options => rel_site)
        rel.user = users(:bob)
        rel.save!
        rel.check
        event = Huginn::Event.last
      ...
      describe "XML" do
        before do
          ...
          @checker = Huginn::Agents::WebsiteAgent.new(name: 'github', options: {
            'name' => 'GitHub',
            'expected_update_period_in_days' => '2',
            'type' => 'xml',
        ...
        it "works with XPath" do
          expect {
            @checker.check
          }.to change { Huginn::Event.count }.by(20)
          event = Huginn::Event.last
        ...
        it "works with XPath with namespaces unstripped" do
          ...
          expect {
            @checker.check
          }.to change { Huginn::Event.count }.by(0)
      ...
      describe "XML with cdata" do
        before do
          ...
          @checker = Huginn::Agents::WebsiteAgent.new(name: 'cdata', options: {
            'name' => 'CDATA',
            'expected_update_period_in_days' => '2',
            'type' => 'xml',
        ...
        it "works with XPath" do
          expect {
            @checker.check
          }.to change { Huginn::Event.count }.by(10)
          event = Huginn::Event.last
      ...
      describe "JSON" do
        ...
        it "can handle arrays" do
          ...
          (event2, event1) = Huginn::Event.last(2)
...
fire: hot
          ...
          @checker = Huginn::Agents::WebsiteAgent.new(name: 'Text Site', options: site)
          @checker.user = users(:bob)
          @checker.save!
        end
        ...
        it "works with regexp with named capture" do
          ...
          event1, event2 = Huginn::Event.last(2)
    ...
    describe "#receive" do
      describe "with a url or url_from_event" do
        ...
        it "should interpolate values from incoming event payload" do
          ...
          expect(Huginn::Event.last.payload).to eq({
            'from' => 'http://foo.com',
        ...
        it "should use the options url if no url is in the event payload, and `url_from_event` is not provided" do
          ...
          expect(Huginn::Event.last.payload['title']).to eq('Evolving')
          expect(Huginn::Event.last.payload['link']).to eq('Random')
        end
        ...
        it "should interpolate values from incoming event payload and _response_" do
          ...
          expect(Huginn::Event.last.payload['response_info']).to eq('The reponse from XKCD was 200 OK.')
        end
        ...
        it "should support merging of events" do
          ...
          last_payload = Huginn::Event.last.payload
      ...
      describe "with a data_from_event" do
        describe "with json data" do
          ...
          it "should output an error when nothing can be found at the path" do
            ...
            expect {
              @checker.receive([@event])
            }.to_not change { Huginn::Event.count }
  ...
  describe "checking with http basic auth" do
    before do
      ...
      @checker = Huginn::Agents::WebsiteAgent.new(:name => "auth", :options => @valid_options)
      @checker.user = users(:bob)
      @checker.save!
  ...
  describe "checking with headers" do
    before do
      ...
      @checker = Huginn::Agents::WebsiteAgent.new(:name => "ua", :options => @valid_options)
      @checker.user = users(:bob)
      @checker.save!
    end
  ...
  describe "checking urls" do
    ...
    describe "#check" do
      before do
        expect { @checker.check }.to change { Huginn::Event.count }.by(7)
        @events = Huginn::Event.last(7)
      end
