...
describe Huginn::Agents::BasecampAgent do
  it_behaves_like Huginn::Oauthable
  ...
  before(:each) do
    ...
    @checker = Huginn::Agents::BasecampAgent.new(:name => "somename", :options => @valid_params)
    @checker.service = huginn_services(:generic)
    @checker.user = users(:jane)
    @checker.save!

    stub(huginn_services(:generic)).refresh_token!
  end
