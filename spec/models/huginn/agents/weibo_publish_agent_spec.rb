...
describe Huginn::Agents::WeiboPublishAgent do
  before do
    @opts = {
      :uid => "1234567",
    ...
    @checker = Huginn::Agents::WeiboPublishAgent.new(:name => "Weibo Publisher", :options => @opts)
    ...
    @sent_messages = []
    stub.any_instance_of(Huginn::Agents::WeiboPublishAgent).publish_tweet { |message| @sent_messages << message}
  end
  ...
  describe '#receive' do
    it 'should publish any payload it receives' do
      ...
      Huginn::Agents::WeiboPublishAgent.async_receive(@checker.id, [event1.id, event2.id])
  ...
  describe '#receive a tweet' do
    it 'should publish a tweet after expanding any t.co urls' do
      event = Huginn::Event.new
      event.agent = huginn_agents(:bob_twitter_user_agent)
      event.payload = JSON.parse(File.read(Rails.root.join("spec/data_fixtures/one_tweet.json")))
      event.save!

      Huginn::Agents::WeiboPublishAgent.async_receive(@checker.id, [event.id])
