...
describe Huginn::Agents::TwitterFavorites do
  before do
  ...
  before do
    ...
    @agent = Huginn::Agents::TwitterFavorites.new(name: "tectonic", options: @opts)
    ...
    @event.agent = huginn_agents(:tectonic_twitter_user_agent)
  ...
  describe "making sure check method works" do
    it "expect change method to change event" do
      expect { @agent.check }.to change {Huginn::Event.count}.by(3)
    end
  end
  ...
  describe "#check with starting_at=future date" do
    it "should check for changes starting_at a future date, thus not find any" do
      ...
      @agent1 = Huginn::Agents::TwitterFavorites.new(name: "tectonic", options: opts)
      @agent1.service = huginn_services(:generic)
      @agent1.user = users(:bob)
      @agent1.save!

      expect { @agent1.check }.to change { Huginn::Event.count }.by(0)
    end
  end
end
