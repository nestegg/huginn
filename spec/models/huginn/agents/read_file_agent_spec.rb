...
describe Huginn::Agents::ReadFileAgent do
  before(:each) do
    @valid_params = {
                      'data_key' => 'data',
                    }

    @checker = Huginn::Agents::ReadFileAgent.new(:name => 'somename', :options => @valid_params)
  ...
  context '#receive' do
    it "emits an event with the contents of the receives files" do
      event = Huginn::Event.new(payload: {file_pointer: {agent_id: 111, file: 'test'}})
      io_mock = mock()
      mock(@checker).get_io(event) { StringIO.new("testdata") }
      expect { @checker.receive([event]) }.to change(Huginn::Event, :count).by(1)
      expect(Huginn::Event.last.payload).to eq('data' => 'testdata')
    end
  end
end
