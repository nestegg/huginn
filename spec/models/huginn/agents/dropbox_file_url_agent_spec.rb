...
describe Huginn::Agents::DropboxFileUrlAgent do
  before(:each) do
    @agent = Huginn::Agents::DropboxFileUrlAgent.new(
  ...
  describe "#receive" do
    ...
    def create_event(payload)
      event = Huginn::Event.new(payload: payload)
      event.agent = huginn_agents(:bob_manual_event_agent)
      event.save!
      event
    end
    ...
    context 'with a single path' do
      ...
      it 'creates one event with the temporary dropbox link' do
        expect { @agent.receive([@event]) }.to change(Huginn::Event, :count).by(1)
        expect(Huginn::Event.last.payload).to eq(first_dropbox_url_payload)
      end
    ...
    context 'with multiple comma-separated paths' do
      ...
      it 'creates one event with the temporary dropbox link for each path' do
        expect { @agent.receive([@event]) }.to change(Huginn::Event, :count).by(3)
        last_events = Huginn::Event.last(3)
