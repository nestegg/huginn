...
describe Huginn::Agents::JabberAgent do
  let(:sent) { [] }
  let(:config) {
    {
  ...
  let(:agent) do
    Huginn::Agents::JabberAgent.new(name: 'Jabber Agent', options: config).tap do |a|
      a.user = users(:bob)
      a.save!
    end
  end
  ...
  let(:event) do
    Huginn::Event.new.tap do |e|
      e.agent = huginn_agents(:bob_weather_agent)
  ...
  before do
    stub.any_instance_of(Huginn::Agents::JabberAgent).deliver { |message| sent << message }
  end

  describe "#working?" do
    it "checks if events have been received within the expected receive period" do
      expect(agent).not_to be_working # No events received
      Huginn::Agents::JabberAgent.async_receive agent.id, [event.id]
  ...
  describe "receive" do
    it "should send an IM for each event" do
      event2 = Huginn::Event.new.tap do |e|
        e.agent = huginn_agents(:bob_weather_agent)
  ...
  describe Huginn::Agents::JabberAgent::Worker do
    before(:each) do
      @worker = Huginn::Agents::JabberAgent::Worker.new(agent: agent)
      @worker.setup
