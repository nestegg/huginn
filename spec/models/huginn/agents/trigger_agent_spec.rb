...
describe Huginn::Agents::TriggerAgent do
  before do
    @valid_params = {
    ...
    @checker = Huginn::Agents::TriggerAgent.new(@valid_params)
  ...
  describe "#working?" do
    it "checks to see if the Agent has received any events in the last 'expected_receive_period_in_days' days" do
      ...
      Huginn::Agents::TriggerAgent.async_receive(@checker.id, [@event.id])
  ...
  describe "#receive" do
    ...
    it "puts can extract values into the message based on paths" do
      @checker.receive([@event])
      expect(Huginn::Event.last.payload['message']).to eq("I saw 'a2b' from Joe")
    end
    ...
    it "handles multiple events" do
      ...
      event3 = Huginn::Event.new
      event3.agent = huginn_agents(:bob_weather_agent)
    ...
    describe "with multiple rules" do
      ...
      it "can accept a partial rule set match when 'must_match' is present and less than the total number of rules" do
        ...
        expect {
          @checker.receive([@event])
        }.to change { Huginn::Event.count }         # but the first one matches
        ...
        expect {
          @checker.receive([@event])
        }.not_to change { Huginn::Event.count }     # only 1 matches, we needed 2
      end
    end
