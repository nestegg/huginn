...
describe Huginn::Agents::TwitterSearchAgent do
  before do
  ...
  let(:checker) {
    _checker = Huginn::Agents::TwitterSearchAgent.new(name: "search freebandnames", options: @opts)
    _checker.service = huginn_services(:generic)
  ...
  describe "#check with starting_at=future date" do
    it "should check for changes starting_at a future date, thus not find any" do
      ...
      checker = Huginn::Agents::TwitterSearchAgent.new(name: "search freebandnames", options: opts)
