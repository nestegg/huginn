...
describe Huginn::Agents::TwitterStreamAgent do
  before do
    @opts = {
      :consumer_key => "---",
      :consumer_secret => "---",
    ...
    @agent = Huginn::Agents::TwitterStreamAgent.new(:name => "HuginnBot", :options => @opts)
  ...
  context "#setup_worker" do
    it "ensures the dependencies are available" do
      mock(STDERR).puts(Huginn::Agents::TwitterStreamAgent.twitter_dependencies_missing)
      mock(Huginn::Agents::TwitterStreamAgent).dependencies_missing? { true }
      expect(Huginn::Agents::TwitterStreamAgent.setup_worker).to eq(false)
    end

    it "returns now workers if no agent is active" do
      mock(Huginn::Agents::TwitterStreamAgent).active { [] }
      expect(Huginn::Agents::TwitterStreamAgent.setup_worker).to eq([])
    end

    it "returns a worker for an active agent" do
      mock(Huginn::Agents::TwitterStreamAgent).active { [@agent] }
      workers = Huginn::Agents::TwitterStreamAgent.setup_worker
      expect(workers).to be_a(Array)
      expect(workers.length).to eq(1)
      expect(workers.first).to be_a(Huginn::Agents::TwitterStreamAgent::Worker)
    ...
    it "correctly maps keywords to agents" do
      ...
      mock(Huginn::Agents::TwitterStreamAgent).active { [@agent, agent2] }
  ...
  describe Huginn::Agents::TwitterStreamAgent::Worker do
    before(:each) do
      @mock_agent = mock!
      @config = {agent: @agent, config: {filter_to_agent_map: {'agent' => [@mock_agent]}}}
      @worker = Huginn::Agents::TwitterStreamAgent::Worker.new(@config)
      @worker.instance_variable_set(:@recent_tweets, [])
      #mock(@worker).schedule_in(Huginn::Agents::TwitterStreamAgent::Worker::RELOAD_TIMEOUT)
