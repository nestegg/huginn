  ...
  describe Event do
    describe ".with_location" do
      it "selects events with location" do
        event = huginn_events(:bob_website_agent_event)
        event.lat = 2
        event.lng = 3
        event.save!
    ...
    describe "#reemit" do
      it "creates a new event identical to itself" do
        huginn_events(:bob_website_agent_event).lat = 2
        huginn_events(:bob_website_agent_event).lng = 3
        huginn_events(:bob_website_agent_event).created_at = 2.weeks.ago
        expect {
          huginn_events(:bob_website_agent_event).reemit!
        }.to change { Event.count }.by(1)
        expect(Event.last.payload).to eq(huginn_events(:bob_website_agent_event).payload)
        expect(Event.last.agent).to eq(huginn_events(:bob_website_agent_event).agent)
    ...
    describe ".cleanup_expired!" do
      it "removes any Events whose expired_at date is non-null and in the past, updating Agent counter caches" do
        half_hour_event = huginn_agents(:jane_weather_agent).create_event :expires_at => 20.minutes.from_now
        one_hour_event = huginn_agents(:bob_weather_agent).create_event :expires_at => 1.hours.from_now
        two_hour_event = huginn_agents(:jane_weather_agent).create_event :expires_at => 2.hours.from_now
        three_hour_event = huginn_agents(:jane_weather_agent).create_event :expires_at => 3.hours.from_now
        non_expiring_event = huginn_agents(:bob_weather_agent).create_event({})

        initial_bob_count = huginn_agents(:bob_weather_agent).reload.events_count
        initial_jane_count = huginn_agents(:jane_weather_agent).reload.events_count
        ...
        expect(huginn_agents(:bob_weather_agent).reload.events_count).to eq(initial_bob_count)
        expect(huginn_agents(:jane_weather_agent).reload.events_count).to eq(initial_jane_count)
        ...
        expect(Event.find_by_id(three_hour_event.id)).not_to be_nil
        expect(Event.find_by_id(non_expiring_event.id)).not_to be_nil
        expect(huginn_agents(:bob_weather_agent).reload.events_count).to eq(initial_bob_count - 1)
        expect(huginn_agents(:jane_weather_agent).reload.events_count).to eq(initial_jane_count - 1)
        ...
        expect(Event.find_by_id(non_expiring_event.id)).not_to be_nil
        expect(huginn_agents(:bob_weather_agent).reload.events_count).to eq(initial_bob_count - 1)
        expect(huginn_agents(:jane_weather_agent).reload.events_count).to eq(initial_jane_count - 2)
      end

      it "doesn't touch Events with no expired_at" do
        event = Event.new
        event.agent = huginn_agents(:jane_weather_agent)
    ...
    describe "after destroy" do
      it "nullifies any dependent AgentLogs" do
        expect(huginn_agent_logs(:log_for_jane_website_agent).outbound_event_id).to be_present
        expect(huginn_agent_logs(:log_for_bob_website_agent).outbound_event_id).to be_present

        huginn_agent_logs(:log_for_bob_website_agent).outbound_event.destroy

        expect(huginn_agent_logs(:log_for_jane_website_agent).reload.outbound_event_id).to be_present
        expect(huginn_agent_logs(:log_for_bob_website_agent).reload.outbound_event_id).to be_nil
      end
    end
    ...
    describe "caches" do
      describe "when an event is created" do
        it "updates a counter cache on agent" do
          expect {
            huginn_agents(:jane_weather_agent).events.create!(:user => users(:jane))
          }.to change { huginn_agents(:jane_weather_agent).reload.events_count }.by(1)
        end

        it "updates last_event_at on agent" do
          expect {
            huginn_agents(:jane_weather_agent).events.create!(:user => users(:jane))
          }.to change { huginn_agents(:jane_weather_agent).reload.last_event_at }
        end
      end

      describe "when an event is updated" do
        it "does not touch the last_event_at on the agent" do
          event = huginn_agents(:jane_weather_agent).events.create!(:user => users(:jane))

          huginn_agents(:jane_weather_agent).update_attribute :last_event_at, 2.days.ago

          expect {
            event.update_attribute :payload, { 'hello' => 'world' }
          }.not_to change { huginn_agents(:jane_weather_agent).reload.last_event_at }
        end
      end
    end
  end
