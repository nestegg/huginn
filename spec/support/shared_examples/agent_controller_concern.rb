...
module Huginn
  shared_examples_for AgentControllerConcern do
    ...
    describe "validation" do
      describe "of action" do
        ...
        it "should accept 'run' if all target agents are schedulable" do
          agent.control_targets = [huginn_agents(:bob_website_agent)]
          expect(agent).to be_valid
        end

        it "should reject 'run' if targets include an unschedulable agent" do
          agent.control_targets = [huginn_agents(:bob_rain_notifier_agent)]
          expect(agent).not_to be_valid
        end
    ...
    describe "control!" do
      before do
        agent.control_targets = [huginn_agents(:bob_website_agent), huginn_agents(:bob_weather_agent)]
        agent.save!
      end

      it "should run targets" do
        control_target_ids = agent.control_targets.map(&:id)
        stub(Huginn::Agent).async_check(anything) { |id|
      ...
      it "should configure targets" do
        ...
        agent.save!
        old_options = huginn_agents(:bob_website_agent).options

        agent.control!

        expect(agent.control_targets.reload).to all(satisfy { |a| a.options['url'] == 'http://some-new-url.com/SOMETHING' })
        expect(huginn_agents(:bob_website_agent).reload.options).to eq(old_options.merge('url' => 'http://some-new-url.com/SOMETHING'))
      end

      it "should configure targets with nested objects" do
        agent.control_targets << huginn_agents(:bob_data_output_agent)
        agent.options['action'] = 'configure'
        agent.options['configure_options'] = {
          template: {
            item: {
        ...
        agent.save!
        old_options = huginn_agents(:bob_data_output_agent).options
        ...
        expect(huginn_agents(:bob_data_output_agent).reload.options).to eq(old_options.deep_merge(agent.options['configure_options']))
      end
    end
  end
end
