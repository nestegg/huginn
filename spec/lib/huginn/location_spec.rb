...
describe Huginn::Location do
  ...
  it "considers a location empty if either latitude or longitude is missing" do
    expect(Huginn::Location.new.empty?).to be_truthy
    expect(Huginn::Location.new(lat: 2, radius: 1).present?).to be_falsy
    expect(Huginn::Location.new(lng: 3, radius: 1).present?).to be_falsy
  end
