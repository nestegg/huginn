...
describe Huginn::AgentRunner do
  context "without traps" do
    before do
      stub.instance_of(Rufus::Scheduler).every
      stub.instance_of(Huginn::AgentRunner).set_traps
      @agent_runner = Huginn::AgentRunner.new
    end

    after(:each) do
      @agent_runner.stop
      Huginn::AgentRunner.class_variable_set(:@@agents, [])
    end
    ...
    context "#load_workers" do
      before do
        Huginn::AgentRunner.class_variable_set(:@@agents, [HuginnScheduler, DelayedJobWorker])
      end
      ...
      it "loads only the workers specified in the :only option" do
        agent_runner = Huginn::AgentRunner.new(only: HuginnScheduler)
        workers = agent_runner.send(:load_workers)
      ...
      it "does not load workers specified in the :except option" do
        agent_runner = Huginn::AgentRunner.new(except: HuginnScheduler)
        workers = agent_runner.send(:load_workers)
    ...
    context "running workers" do
      ...
      context "#run_workers" do
        ...
        it "kills no long active workers" do
          ...
          mock.instance_of(DelayedJobWorker).run!
          @agent_runner.send(:run_workers)
          Huginn::AgentRunner.class_variable_set(:@@agents, [DelayedJobWorker])
