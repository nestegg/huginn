...
describe Huginn::DryRunnable do
  class Huginn::Agents::SandboxedAgent < Huginn::Agent
  ...
  before do
    stub(Huginn::Agents::SandboxedAgent).valid_type?("Huginn::Agents::SandboxedAgent") { true }

    @agent = Huginn::Agents::SandboxedAgent.create(name: "some agent") { |agent|
  ...
  describe "dry_run!" do
    ...
    it "traps any destructive operations during a run when an event is given" do
      ...
      expect {
        results = @agent.dry_run!(Huginn::Event.new(payload: { 'prefix' => 'super' }))
        @agent.reload
      }.not_to change {
        [@agent.memory, counts]
      }
