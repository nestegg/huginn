...
describe Huginn::LongRunnable do
  class Huginn::LongRunnableAgent < Huginn::Agent
  ...
  before(:each) do
    @agent = Huginn::LongRunnableAgent.new
  end
  ...
  it "should build the worker_id" do
    expect(@agent.worker_id).to eq('Huginn::LongRunnableAgent--bf21a9e8fbc5a3846fb05b4fa0859e0917b2202f')
  end

  context "#setup_worker" do
    it "returns active agent workers" do
      mock(Huginn::LongRunnableAgent).active { [@agent] }
      workers = Huginn::LongRunnableAgent.setup_worker
      expect(workers.length).to eq(1)
      expect(workers.first).to be_a(Huginn::LongRunnableAgent::Worker)
      expect(workers.first.agent).to eq(@agent)
    end

    it "returns an empty array when no agent is active" do
      mock(Huginn::LongRunnableAgent).active { [] }
      workers = Huginn::LongRunnableAgent.setup_worker
      expect(workers.length).to eq(0)
    end
  end

  describe Huginn::LongRunnable::Worker do
    before(:each) do
      @agent = Object.new
      @worker = Huginn::LongRunnable::Worker.new(agent: @agent, id: 'test1234')
