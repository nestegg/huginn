...
describe Huginn::SortableEvents do
  let(:agent_class) {
    Class.new(Huginn::Agent) do
      include Huginn::SortableEvents
  ...
  describe 'sort_events' do
    ...
    let(:events) {
      payloads.map { |payload| Huginn::Event.new(payload: payload) }
    }
  ...
  describe 'automatic event sorter' do
    ...
    describe 'behavior' do
      class Huginn::Agents::EventOrderableAgent < Huginn::Agent
        include Huginn::SortableEvents
      ...
      def new_agent(events_order = nil)
        options = {}
        options['events_order'] = events_order if events_order
        Huginn::Agents::EventOrderableAgent.new(name: 'test', options: options) { |agent|
          agent.user = users(:bob)
      ...
      it 'should keep the order of created events unless events_order is specified' do
        [[], [nil], [[]]].each do |args|
          ...
          agent.save!
          expect { agent.check }.to change { Huginn::Event.count }.by(4)
      ...
      it 'should sort events created in receive() in the order specified in events_order' do
        ...
        agent.save!
        expect {
          agent.receive([Huginn::Event.new(payload: { 'title_suffix' => ' [new]' }),
                         Huginn::Event.new(payload: { 'title_suffix' => ' [popular]' })])
        }.to change { Huginn::Event.count }.by(8)
