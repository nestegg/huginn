...
describe WebRequestsController do
  class Huginn::Agents::WebRequestReceiverAgent < Huginn::Agent
  ...
  before do
    stub(Huginn::Agents::WebRequestReceiverAgent).valid_type?("Huginn::Agents::WebRequestReceiverAgent") { true }
    @agent = Huginn::Agents::WebRequestReceiverAgent.new(:name => "something", :options => { :secret => "my_secret" })
  ...
  describe "legacy update_location endpoint" do
    before do
      @agent = Huginn::Agent.build_for_type("Huginn::Agents::UserLocationAgent", users(:bob), name: "something", options: { secret: "my_secret" })
      @agent.save!
    end
    ...
    it "should only consider Agents::UserLocationAgents for the given user" do
      @jane_agent = Huginn::Agent.build_for_type("Huginn::Agents::UserLocationAgent", users(:jane), name: "something", options: { secret: "my_secret" })
      @jane_agent.save!
    ...
    it "should only look at agents with the given secret" do
      @agent2 = Huginn::Agent.build_for_type("Huginn::Agents::UserLocationAgent", users(:bob), name: "something", options: { secret: "my_secret2" })
      @agent2.save!
