...
describe Huginn::JobsController do
  describe "GET index" do
    before do
      async_handler_yaml =
        "--- !ruby/object:ActiveJob::QueueAdapters::DelayedJobAdapter::JobWrapper\njob_data:\n  job_class: AgentCheckJob\n  job_id: 123id\n  queue_name: default\n  arguments:\n  - %d\n"

      Delayed::Job.create!(handler: async_handler_yaml % [huginn_agents(:jane_website_agent).id])
      Delayed::Job.create!(handler: async_handler_yaml % [huginn_agents(:bob_website_agent).id])
      Delayed::Job.create!(handler: async_handler_yaml % [huginn_agents(:jane_weather_agent).id])
      huginn_agents(:jane_website_agent).destroy
      Delayed::Job.create!(handler: async_handler_yaml % [huginn_agents(:bob_weather_agent).id], locked_at: Time.now, locked_by: 'test')
