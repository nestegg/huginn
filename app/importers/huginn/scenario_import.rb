...
module Huginn
  class ScenarioImport
    ...
    DANGEROUS_AGENT_TYPES = %w[Huginn::Agents::ShellCommandAgent]
    URL_REGEX = /\Ahttps?:\/\//i
    ...
    def import(options = {})
      ...
      unless options[:skip_agents]
        created_agents = agent_diffs.map do |agent_diff|
          agent = agent_diff.agent || Huginn::Agent.build_for_type("Huginn::Agents::" + agent_diff.type.incoming, user)
    ...
    class AgentDiff < OpenStruct
      ...
      def agent_instance
        "Huginn::Agents::#{self.type.updated}".constantize.new
      end
    end
  end
end
