module Huginn::Agents
  class EventFormattingAgent < Huginn::Agent
    cannot_be_scheduled!
    can_dry_run!
    ...
    description <<-MD
      ...
      The upstream agent of each received event is accessible via the key `agent`, which has the following attributes: #{''.tap { |s| s << Huginn::AgentDrop.instance_methods(false).map { |m| "`#{m}`" }.join(', ') }}.
