module Huginn::Agents
  class ShellCommandAgent < Huginn::Agent
    ...
    description <<-MD
      ...
      *Warning*: This type of Agent runs arbitrary commands on your system, #{ShellCommandAgent.should_run? ? "but is **currently enabled**" : "and is **currently disabled**"}.
    ...
    def working?
      Huginn::Agents::ShellCommandAgent.should_run? && event_created_within?(interpolated['expected_update_period_in_days']) && !recent_error_logs?
    end
    ...
    def handle(opts, event = nil)
      if Huginn::Agents::ShellCommandAgent.should_run?
