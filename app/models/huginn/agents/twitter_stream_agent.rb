module Huginn::Agents
  class TwitterStreamAgent < Huginn::Agent
    include Huginn::TwitterConcern
    ...
    def self.setup_worker
      Huginn::Agents::TwitterStreamAgent.active.group_by { |agent| agent.twitter_oauth_token }.map do |oauth_token, agents|
        if Huginn::Agents::TwitterStreamAgent.dependencies_missing?
          STDERR.puts Huginn::Agents::TwitterStreamAgent.twitter_dependencies_missing
          STDERR.flush
          return false
        end
    ...
    class Worker < Huginn::LongRunnable::Worker
      ...
      def handle_status(status)
        ...
        @filter_to_agent_map.keys.each do |filter|
          ...
          @filter_to_agent_map[filter].each do |agent|
            ...
            Huginn::AgentRunner.with_connection do
