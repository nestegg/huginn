module Huginn
  class AgentsController < ApplicationController
    ...
    def create
      ...
      respond_to do |format|
        if @agent.save
          format.html { redirect_back "'#{@agent.name}' was successfully created.", return: huginn_agents_path }
          format.json { render json: @agent, status: :ok, location: huginn_agent_path(@agent) }
        else
          initialize_presenter
    ...
    def update
      ...
      respond_to do |format|
        if @agent.update_attributes(params[:agent])
          format.html { redirect_back "'#{@agent.name}' was successfully updated.", return: huginn_agents_path }
          format.json { render json: @agent, status: :ok, location: huginn_agent_path(@agent) }
        else
          initialize_presenter
    ...
    def redirect_back(message, options = {})
      if path = filtered_agent_return_link(options)
        ...
        super huginn_agents_path, notice: message
      end
    end

    def build_agent
      @agent = Huginn::Agent.build_for_type(params[:agent].delete(:type),
                                    current_user,
                                    params[:agent])
    end
    ...
    def initialize_presenter
      if @agent.present? && @agent.is_form_configurable?
        @agent = Huginn::FormConfigurableAgentPresenter.new(@agent, view_context)
      end
    end
  end
end
